...
技術情報を自動収集し、AIで分類・要約してSlackに配信するシステム。
...
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  【Fetch RSS WF】           【Process RSS Articles WF】    【Send Slack WF】     │
│  Schedule: 1時間ごと        (サブワークフロー)              Schedule: 3時間ごと  │
│                                                                                 │
│  ┌──────────────┐          ┌───────────────────┐         ┌──────────────────┐  │
...
│         ▼                  │Analyze with AI    │                  ▼            │
│  ┌──────────────┐          │(OpenAI)           │         ┌──────────────────┐  │
│  │Fetch RSS     │          │・カテゴリ判定      │         │Generate Slack    │  │
│  │Feed          │          │・キーワード抽出    │         │Message (AI)      │  │
│  └──────┬───────┘          │・要約生成         │         └────────┬─────────┘  │
│         ▼                  │・タイトル翻訳      │                  ▼            │
│  ┌──────────────┐          └─────────┬─────────┘         ┌──────────────────┐  │
│  │Process RSS   │──────────►        ▼                   │Send to Slack     │  │
│  │Articles      │          ┌───────────────────┐         └──────────────────┘  │
│  │(サブWF呼出)   │          │Save Article       │                              │
│  └──────┬───────┘          │(Supabase保存)     │                              │
│         ▼                  └───────────────────┘                              │
